...
ElecState eState = PREOPEN, lastEState = (ElecState)255;

struct ElecWindow
{
    // ตั้ง "วัน/เวลาเริ่ม" และ "สิ้นสุด" ที่นี่ (แก้ให้ตรงกับงานจริง)
    // ตัวอย่าง: เริ่ม 09:00–16:00 วันที่ 2025-01-15
    DateTime start = DateTime(2025, 1, 15, 9, 0, 0);
    DateTime end = DateTime(2025, 1, 15, 16, 0, 0);
} EWIN;

unsigned long lastRTCpoll = 0;
const unsigned long RTC_POLL_MS = 500; // อัปเดตทุก 0.5s

// ข้อความสั้น ๆ ขึ้น TM1638 (8 ตัว)
const char *TXT_PRE = "PREOPEN";
const char *TXT_OPEN = "OPEN   ";
const char *TXT_END = "CLOSED ";
...
public:
    ...
    void update()
    {
        ...
        if (phase == 1 && (long)(now - phaseEnd) >= 0)
        { // end ON -> GAP
#if BUZZER_PASSIVE
            noTone(BUZZER_PIN);
#else
        ...
        if (phase == 2 && (long)(now - phaseEnd) >= 0)
        { // next
            idx++;
            phase = 0;
            if (idx >= count)
...
// ---- [ADD] แสดงข้อความ 8 ตัวบน TM1638 (กัน NUL/สั้น/ยาว) ----
void tmShow8(const char *s)
{
    char buf[9];
    uint8_t i = 0;
    for (; i < 8 && s[i]; ++i)
...
// ---- [ADD] ฟังก์ชันคำนวณสถานะ + ส่งออกจอ/Serial เมื่อมีการเปลี่ยน ----
void updateElectionState()
{
    unsigned long nowms = millis();
    if (nowms - lastRTCpoll < RTC_POLL_MS)
        return;
    lastRTCpoll = nowms;
    ...
    if (now < EWIN.start)
        eState = PREOPEN;
    else if (now <= EWIN.end)
        eState = OPEN;
    else
        eState = CLOSED;

    // อัปเดต TM1638 + ส่ง Serial เฉพาะเมื่อ "สถานะ" เปลี่ยน
    if (eState != lastEState)
    {
        ...
        {
            tmShow8(TXT_PRE);
            // อาจเปิด LED สีแดง 1 ดวงบอก “รอ” (ตำแหน่ง 1)
            tm.setLED(TM1638_COLOR_RED, 1);
            tm.setLED(TM1638_COLOR_NONE, 2);
            tm.setLED(TM1638_COLOR_NONE, 3);
            Serial.println(F("STATE:PREOPEN"));
        }
        else if (eState == OPEN)
        {
            tmShow8(TXT_OPEN);
            // เปิด LED สีเขียวบอก “กำลังเปิด” (ตำแหน่ง 2)
            tm.setLED(TM1638_COLOR_NONE, 1);
            tm.setLED(TM1638_COLOR_GREEN, 2);
            tm.setLED(TM1638_COLOR_NONE, 3);
        ...
        {
            tmShow8(TXT_END);
            // เปิด LED สีแดงหรือปิดทั้งหมดก็ได้ (เอาตามชอบ)
    ...
    // (ออปชัน) แสดง “นับถอยหลัง” ระหว่าง PREOPEN/OPEN
    // ใช้ตัวเลขอย่างง่ายบน TM1638 เช่น เหลือวินาที / “HHMMSS”
    // คุณจะปรับเป็นรูปแบบอื่นก็ได้
    long remain = 0;
    if (eState == PREOPEN)
        remain = EWIN.start.unixtime() - now.unixtime();
    else if (eState == OPEN)
        remain = EWIN.end.unixtime() - now.unixtime();

    if (remain < 0)
        remain = 0;
    if (eState != CLOSED)
    {
        // แสดงนาทีวินาทีแบบ MMSS (พอดี 4 หลัก; แต่ TM มี 8 หลัก → จัดกึ่งกลาง)
        long mm = remain / 60;
        long ss = remain % 60;
        long mmss = (mm % 100) * 100 + ss; // สูงสุด 99:59
        // แสดงเลื่อนชิดขวา 4 หลักท้าย
        tm.displayIntNum(mmss, true, 4, 4); // (value, leadingZeros, length, pos0-7)
    }
}
...
void setup()
{
    ...
    if (!rtc.begin())
    {
        // แจ้ง ESP32 ว่า RTC พัง จะได้เข้าระบบ Manual ได้
        Serial.println(F("STATE:RTCERR"));
    }
    else
    {
        // NOTE: ไม่ adjust เวลาอัตโนมัติที่นี่ (เราจะสั่ง SETTIME จาก ESP32 หรือ web)
    }

    // ==== [ADD] TM1638 start ====
    tm.displayBegin(); // init ชิป TM1638
    tm.setLED(TM1638_COLOR_NONE, 1);
    tm.setLED(TM1638_COLOR_NONE, 2);
    tm.setLED(TM1638_COLOR_NONE, 3);
    tm.displayText("PREOPEN "); // ข้อความเริ่มต้น
...
void loop()
{
    ...
    // ==== [ADD] ปุ่ม TM1638 เป็นทางลัดผู้ดูแล (FORCE) ====
    static uint8_t lastBtns = 0;
    uint8_t btns = tm.readButtons();
    uint8_t rising = (btns & ~lastBtns);
    lastBtns = btns;

    // K1 กด -> บังคับเปิด (OPEN:FORCE)
    if (rising & 0x01)
    {
        Serial.println(F("STATE:OPEN:FORCE"));
        // แสดงบน TM และ LED ให้เห็นชัด
        tm.displayText("OPEN   ");
        tm.setLED(TM1638_COLOR_NONE, 1);
        tm.setLED(TM1638_COLOR_GREEN, 2);
        tm.setLED(TM1638_COLOR_NONE, 3);
    ...
    // K2 กด -> บังคับปิด (CLOSED:FORCE)
    if (rising & 0x02)
    {
        Serial.println(F("STATE:CLOSED:FORCE"));
        tm.displayText("CLOSED ");
        tm.setLED(TM1638_COLOR_NONE, 1);
        tm.setLED(TM1638_COLOR_NONE, 2);
        tm.setLED(TM1638_COLOR_RED, 3);
    }
