    ...
    if (httpCode == 200) {
      ...
      while (millis() - waitStart < 5000) {
        if (mySerial.available()) {
          ...
          if (line.indexOf("RESET_OK") >= 0 || line.indexOf("CLEARED") >= 0 || line.indexOf("SUCCESS") >= 0 || line.indexOf("OK") >= 0) {
...
void normalScanFlow() {
  ...
  // for (int ii = 0; ii < 5; ii++)
  mySerial.println("SSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSS");  // โปรโตคอลตามเดิม
  showUIx(UI_SCAN_CARD, "ยื่นบัตรใกล้เครื่องอ่าน", TR_NONE);
  ...
  // วนรออีเวนต์: CF:xx / SEL:xx / SENDING / VOTE:OK / VOTE:ERR (ไม่จำกัดเวลา)
  bool finished = false;
  bool restartToScanCard = false;

  while (!finished && g_waitingChoice) {
    ...
    while (mySerial.available()) {
      ...
      if (line.startsWith("SEL:")) {
        ...
        if (n >= 0 && n <= 99) {
          ...
          isShowingPhoto = true;
          g_selectedCandidate = n;  // เก็บหมายเลขที่เลือก
          uiSetScanning(false);
          showCandidateJpg(n);
          // ยังคงอยู่ใน loop เพื่อรอ CF
        }
        continue;  // ยังคงอยู่ใน voting loop เพื่อรอ CF command
      }
      ...
      if (line.startsWith("CF:") || line.startsWith(" CF:")) {
        ...
        barStop();
        isShowingPhoto = false;  // หยุดโหมดแสดงรูปทันทีเมื่อได้รับการยืนยัน

        if (g_selectedCandidate < 0 || g_selectedCandidate > 9) {
          showUIx(UI_ERROR, "หมายเลขไม่ถูกต้อง", TR_NONE);
          delay(700);
        ...
        // ถึงจุดนี้ถือว่าโหวตสำเร็จหรือเคยส่งแล้ว → แสดง Thank You และกลับไปสแกนบัตร
  ...
  // ถ้าออกจาก loop โดยไม่ finished หมายถึง user ยกเลิกหรือมีปัญหา
  g_waitingChoice = false;
  isShowingPhoto = false;  // รีเซ็ตโหมดแสดงภาพ
  barStop();
  if (restartToScanCard) {
    uiSetScanning(true);
    showUIx(UI_SCAN_CARD, "ยื่นบัตรใกล้เครื่องอ่าน", TR_NONE);
  } else {
    delay(800);
...
void handleU2Line(const String &raw) {
  ...
  if (m.equalsIgnoreCase("AUTHOK")) {
    ...
    // ถึงจุดนี้โหวตสำเร็จหรือถูกจัดการไว้แล้ว → แสดง Thank You และกลับไปรอการ์ดใหม่
    g_waitingChoice = false;
    g_votePosted = true;
    showUIx(UI_THANKS, "โหวตเสร็จสิ้น", TR_NONE);
    if (!g_receiptPrinted) {
...
void loop() {
  ...
  if (!uiShownScanCard && !inRegisterMode && !inDeleteMode && !inScoreMode && !waitingForPassword && !waitingForDeletePassword && !waitingForScorePassword && !inDeleteMenuMode) {
    showUIx(UI_SCAN_CARD, "ยื่นบัตรใกล้เครื่องอ่าน", TR_NONE);
  ...
  if (!inScoreMode && !waitingForPassword && !waitingForDeletePassword && !waitingForScorePassword && !inDeleteMenuMode) {
    rfid_bus_begin();
    if (rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) {
      cardReady = true;
    }
