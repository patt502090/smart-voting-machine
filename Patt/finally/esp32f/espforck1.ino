...
void setup() {
  ...
  // --- SPI / Bus guard first ---
  // ถ้าใช้ VSPI ตามปกติ: SCK=18, MISO=19, MOSI=23 — CS เราคุมเอง (SD_CS/TFT_CS/SS_PIN)
  SPI.begin(18, 19, 23, SD_CS);
  spi_idle_all();  // ดันทุก CS เป็น HIGH ตั้งแต่เริ่ม

  // --- SD Card mount (ลอง 10MHz → 4MHz) ---
  bool sdOK = false;
  {
    // ปิดอุปกรณ์อื่นจากบัส
    digitalWrite(TFT_CS, HIGH);
    digitalWrite(SS_PIN, HIGH);
  ...
  // --- TFT (ผ่าน TFT_eSPI) + TJpg callback ---
  tft.init();
  tft.setRotation(1);  // แนวนอน 320x240
  TJpgDec.setCallback(tft_output);
  showIdleScreen(sdOK ? "SD OK" : "No SD");

  // --- RC522 init (ผ่าน bus guard เสมอ) ---
  Serial.println("Init RC522...");
  bus_acquire_for_rfid();  // ล็อคบัสให้ RC522 (ฟังก์ชันนี้จะทำ SPI.beginTransaction และดึง SS ลงให้)
  rfid.PCD_Init();
  byte rc522v = rfid.PCD_ReadRegister(MFRC522::VersionReg);
  bus_release_after_rfid();  // ปล่อยบัสคืน
...
void loop() {
  ...
  // ---- อ่านการ์ดแบบล็อคบัส RC522 (กันชนกับ SD/TFT) ----
  bool cardReady = false;
  bus_acquire_for_rfid();  // << จับบัสให้ RC522
  if (rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) {
    cardReady = true;
  }
  bus_release_after_rfid();  // << คืนบัส

  if (cardReady) {
    normalScanFlow();
  }

  // ---- รับคำสั่งจากบอร์ดลูก (ODROID/UNO) ----
  if (mySerial.available()) {
    ...
    if (msg.equalsIgnoreCase("SLEEP!")) {
      ...
      delay(30);
      goDeepSleepNow();  // ไม่กลับ
    }

    // ถ้ามีโหมดสั่งแสดงรูปจาก SD: "SEL:<n>"
    if (msg.startsWith("SEL:")) {
      if (msg.equalsIgnoreCase("SEL:CLEAR")) {
        showIdleScreen("Ready");
      } else {
        int n = msg.substring(4).toInt();
        if (n >= 0 && n <= 99) showCandidateJpg((uint8_t)n);  // โหลด /n.jpg จาก SD
        else showIdleScreen("Bad SEL");
      }
    }
